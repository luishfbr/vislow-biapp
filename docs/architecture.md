...
| **ADR-20** | **O painel de formatação do visual gerado é FECHADO por padrão.** O nó guarda `exposed` — as chaves que o autor publicou —, cada nó publicado vira um `object` no `capabilities.json` com o **id do nó** como `objectName`, e o campo fechado continua saindo **literal** no JSX. | Guardar o que está *travado* daria de graça "ausente = tudo destravado", e com isso todo projeto já salvo passaria a entregar cada cor e cada pixel ao consumidor no próximo export, sem ninguém ter pedido. Fechado por padrão também torna verificável a propriedade que sustenta o resto: **projeto que não publica nada gera o pacote de antes, byte a byte** — sem `objects`, sem `supportsEmptyDataView`, sem `getFormattingModel`. E ignorar override vira **propriedade estrutural**: o valor do autor está no fonte e não há por onde ler o `objects` naquela posição, então não é uma verificação em runtime que alguém possa contornar. `placement` fica de fora por natureza — é o codegen que o lê, para decidir se embrulha os filhos em `CanvasSlot` (`structural: true` no descritor). | Guardar `locked` e expor tudo por padrão (vira o produto de outra pessoa no próximo export, e obriga o autor a travar 80 campos para proteger uma composição); um `object` único com propriedades `nodeId_chave` (nome composto que os dois lados precisam montar e desmontar); vendorizar o `component-registry` no visual e derivar o painel em runtime (põe o Ajv e o catálogo inteiro dentro do pacote, contra o orçamento de bytes). |
