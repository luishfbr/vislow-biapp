...
| **ADR-14** | O **preview não tem seleção por clique**. A seleção vive no painel de árvore. | Envolver cada nó num elemento clicável insere um `div` na cadeia de flex que os gráficos usam para medir altura — o `ResponsiveContainer` mediria outra coisa e o preview deixaria de valer como referência, que é justamente o ADR-04. | Wrapper clicável (quebra o WYSIWYG por dentro, sem sintoma visível); `refs` + overlay (complexidade alta, ganho pequeno). |
