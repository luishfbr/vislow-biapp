...
| **ADR-08** | **Compilação real por usuário, num backend.** O worker recebe a árvore, gera o projeto `pbiviz` e roda `pbiviz package`. **Reverte a ADR-01 e a ADR-05.** | O patch de placeholder trava o produto: o usuário escolhe entre tipos que o runtime já sabe renderizar, nunca cria. Com compilação real, o `capabilities.json` é **gerado por visual** — o usuário declara os campos que o visual dele vai pedir, que é o que faz "começar do zero" ser real. De quebra, a identidade vem do `pbiviz.json` (adeus achados 34/35/36) e o achado 39 some por construção. Gate aprovado em 2026-07-30: Recharts cabe com 378 KB de folga, build em 11,5 s. | Manter o patch (teto de produto); um visual único no AppSource com config no painel de formatação (padrão Deneb — mata a proposta de valor, o usuário deixa de criar). |
